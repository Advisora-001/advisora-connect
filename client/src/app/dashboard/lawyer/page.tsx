...
export default function LawyerDashboard() {
  ...
  const [activeSection, setActiveSection] = useState<
    "dashboard" | "enquiries" | "appointments" | "profile" | "payout" | "wallet"
  >("dashboard");
  ...
  useEffect(() => {
    ...
    fetchLeads(); fetchAppointments(); fetchWallet();
  ...
  async function fetchWallet() { try { const data = await api.getWallet(); setWallet(data.wallet); } catch {} }
  ...
  const handleRequestPayout = async () => {
    ...
    if (!confirm(`Request a payout of ₦${amount.toLocaleString()}?`)) return;
    setActionLoading("payout");
    try { await api.requestPayout(amount); setPayoutAmount(""); await fetchWallet(); alert("Payout request submitted!"); }
    catch (err: any) { alert(err.message || "Failed to request payout"); }
    finally { setActionLoading(null); }
  };
    ...
    <div className="flex min-h-[calc(100vh-64px)]">
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-8 overflow-auto bg-[#F5F7FA]">
        ...
        {activeSection === "dashboard" && (
          <div>
            <div className="mb-8"><h1 className="text-2xl font-bold text-[#1B2A4A] tracking-tight">Dashboard</h1><p className="text-[#667085] mt-1">Welcome back, {user.firstName} {user.lastName}</p></div>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
              <div className="bg-white rounded-xl border border-[#E5EAF0] p-6"><p className="text-sm font-medium text-[#667085] uppercase tracking-wide">Total Leads</p><p className="text-3xl font-bold text-[#1B2A4A] mt-2">{leads.length}</p></div>
              <div className="bg-white rounded-xl border border-[#E5EAF0] p-6"><p className="text-sm font-medium text-[#667085] uppercase tracking-wide">Unpaid Leads</p><p className="text-3xl font-bold text-[#F59E0B] mt-2">{leads.filter((l) => l.paymentStatus === "unpaid").length}</p></div>
              <div className="bg-white rounded-xl border border-[#E5EAF0] p-6"><p className="text-sm font-medium text-[#667085] uppercase tracking-wide">Appointments</p><p className="text-3xl font-bold text-[#5DBB63] mt-2">{appointments.length}</p></div>
              <div className="bg-white rounded-xl border border-[#E5EAF0] p-6"><p className="text-sm font-medium text-[#667085] uppercase tracking-wide">Verification</p><p className={`text-xl font-bold mt-2 capitalize ${profile?.verificationStatus === "verified" ? "text-[#5DBB63]" : "text-[#F59E0B]"}`}>{profile?.verificationStatus || "pending"}</p>{profile?.verificationStatus !== "verified" && <p className="text-sm text-[#667085] mt-1">Profile pending admin review</p>}</div>
            </div>
          </div>
        )}
        ...
        {activeSection === "enquiries" && (
          <div>
            <div className="mb-8"><h1 className="text-2xl font-bold text-[#1B2A4A] tracking-tight">Client Enquiries</h1><p className="text-[#667085] mt-1">Manage incoming client enquiries and booking requests</p></div>
            <div className="bg-white rounded-xl border border-[#E5EAF0]">
              {leads.length === 0 ? (<div className="p-12 text-center text-[#667085]">No enquiries yet.</div>) : (
                <div className="divide-y divide-[#E5EAF0]">
                  {leads.map((lead: any) => (
                    <div key={lead._id} className="p-5 hover:bg-[#F5F7FA] transition-colors">
                      <div className="mb-3">
                        <p className="font-semibold text-[#1B2A4A]">{lead.enquiryMessage?.substring(0, 100)}{lead.enquiryMessage?.length > 100 ? "..." : ""}</p>
                        <p className="text-sm text-[#667085] mt-1">{new Date(lead.createdAt).toLocaleDateString()} • {lead.clientName}</p>
                        {lead.bookingContext && (<div className="mt-2 flex flex-wrap gap-2"><span className="bg-[#EFF6FF] text-[#1E40AF] text-xs px-2 py-1 rounded-full font-medium">{lead.bookingContext.service?.name}</span><span className="bg-[#EEF2F7] text-[#475569] text-xs px-2 py-1 rounded-full font-medium">{lead.bookingContext.date}</span><span className="bg-[#ECFDF5] text-[#166534] text-xs px-2 py-1 rounded-full font-medium">{lead.bookingContext.timeSlot}</span><span className="bg-[#EEF2F7] text-[#667085] text-xs px-2 py-1 rounded-full font-medium capitalize">{lead.bookingContext.consultationType}</span></div>)}
                      </div>
                      {lead.status === "pending" && (<div className="flex gap-3"><button onClick={() => handleRespond(lead._id, "accepted")} disabled={lead.responding} className="px-4 py-2 bg-[#5DBB63] text-white rounded-lg hover:bg-[#4CA652] disabled:opacity-50 font-semibold text-sm transition-colors">Accept</button><button onClick={() => handleRespond(lead._id, "declined")} disabled={lead.responding} className="px-4 py-2 bg-[#EF4444] text-white rounded-lg hover:bg-[#DC2626] disabled:opacity-50 font-semibold text-sm transition-colors">Decline</button></div>)}
                      {lead.status === "accepted" && (<div className="mt-3 p-4 bg-[#ECFDF5] rounded-lg border border-[#5DBB63]/20"><p className="font-semibold text-[#166534] mb-2">Contact Details:</p><p className="text-[#1B2A4A] text-sm">📧 {lead.clientEmail}</p>{lead.clientPhone && <p className="text-[#1B2A4A] text-sm">📱 {lead.clientPhone}</p>}</div>)}
                      {lead.status === "declined" && (<span className="inline-block px-4 py-2 bg-[#FEF2F2] text-[#991B1B] rounded-full font-semibold text-sm">Declined</span>)}
                      {lead.status === "booked" && (<span className="inline-block px-4 py-2 bg-[#EFF6FF] text-[#1E40AF] rounded-full font-semibold text-sm">Booked</span>)}
                    </div>
                  ))}
                </div>
              )}
        ...
        {activeSection === "appointments" && (
          <div>
            <div className="mb-8"><h1 className="text-2xl font-bold text-[#1B2A4A] tracking-tight">Appointments</h1><p className="text-[#667085] mt-1">Manage your consultations and meeting links</p></div>
            <div className="bg-white rounded-xl border border-[#E5EAF0]">
              {appointments.length === 0 ? (<div className="p-12 text-center text-[#667085]">No appointments yet.</div>) : (
                <div className="divide-y divide-[#E5EAF0]">
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${appt.status === 'confirmed' ? 'bg-[#EFF6FF] text-[#1E40AF]' : appt.status === 'completed' ? 'bg-[#ECFDF5] text-[#166534]' : appt.status === 'cancelled' ? 'bg-[#FEF2F2] text-[#991B1B]' : 'bg-[#EEF2F7] text-[#667085]'}`}>{appt.status}</span>
                        ...
                        <div className="flex flex-col gap-2 items-end">
                          {appt.paymentStatus === 'paid' && appt.status !== 'cancelled' && appt.status !== 'completed' && (<>
                            {appt.meetingLink ? (<><a href={appt.meetingLink} target="_blank" rel="noopener noreferrer" className="bg-[#5DBB63] text-white px-4 py-2 rounded-lg hover:bg-[#4CA652] font-semibold text-sm text-center transition-colors">Join Consultation</a><button onClick={() => handleGenerateMeeting(appt._id)} disabled={actionLoading === appt._id} className="text-xs text-[#00A6A6] hover:underline">Regenerate Link</button></>)
                            : (<><button onClick={() => handleGenerateMeeting(appt._id)} disabled={actionLoading === appt._id} className="bg-[#1B2A4A] text-white px-4 py-2 rounded-lg hover:bg-[#16213A] font-semibold text-sm disabled:opacity-50 transition-colors">{actionLoading === appt._id ? 'Generating...' : 'Generate Meet Link'}</button>
                              {showMeetingInput === appt._id ? (<div className="flex gap-2 mt-1"><input type="text" value={meetingLinkInput} onChange={(e) => setMeetingLinkInput(e.target.value)} placeholder="Paste Meet/Zoom link" className="px-3 py-1.5 border border-[#E5EAF0] rounded-lg text-sm w-48" /><button onClick={() => handleSetMeetingLink(appt._id)} disabled={actionLoading === appt._id} className="bg-[#00A6A6] text-white px-3 py-1.5 rounded-lg font-semibold text-sm disabled:opacity-50 transition-colors">Save</button></div>)
                              : (<button onClick={() => { setShowMeetingInput(appt._id); setMeetingLinkInput(''); }} className="text-xs text-[#00A6A6] hover:underline">Add Link Manually</button>)}</>)}
                            <button onClick={() => handleCompleteAppointment(appt._id)} disabled={actionLoading === appt._id} className="bg-[#00A6A6] text-white px-4 py-2 rounded-lg hover:bg-[#008F8F] font-semibold text-sm disabled:opacity-50 transition-colors">{actionLoading === appt._id ? '...' : 'Mark Complete'}</button>
                          </>)}
                          {appt.status === 'completed' && appt.meetingLink && (<a href={appt.meetingLink} target="_blank" rel="noopener noreferrer" className="bg-[#EEF2F7] text-[#667085] px-4 py-2 rounded-lg font-semibold text-sm">View Meeting Link</a>)}
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {activeSection === "profile" && (
          <div>
            <div className="mb-8"><h1 className="text-2xl font-bold text-[#1B2A4A] tracking-tight">Edit Profile</h1><p className="text-[#667085] mt-1">Manage your professional profile and availability</p></div>
            <div className="bg-white rounded-xl border border-[#E5EAF0] p-8 max-w-3xl">
              <form onSubmit={handleProfileUpdate} className="space-y-5">
                <div className="flex items-center gap-6 p-4 bg-[#F5F7FA] rounded-lg border border-[#E5EAF0]">
                  <div className="w-20 h-20 rounded-full overflow-hidden bg-[#1B2A4A] flex items-center justify-center text-white text-2xl font-bold flex-shrink-0">{profileForm.photo ? <img src={profileForm.photo} alt="Profile" className="w-full h-full object-cover" /> : <span>{user.firstName[0]}{user.lastName[0]}</span>}</div>
                  <div className="flex-1"><p className="font-semibold text-[#1B2A4A] mb-2">Profile Photo</p><label className="inline-block bg-[#1B2A4A] text-white px-4 py-2 rounded-lg hover:bg-[#16213A] cursor-pointer font-medium text-sm transition-colors">Upload Photo<input type="file" accept="image/jpeg,image/png,image/jpg" className="hidden" onChange={async (e) => { const file = e.target.files?.[0]; if (!file) return; if (file.size > 5 * 1024 * 1024) { alert("File too large. Maximum size is 5MB."); return; } try { const formData = new FormData(); formData.append("photo", file); const result = await api.uploadPhoto(formData); setProfileForm({ ...profileForm, photo: result.photo }); await refreshUser(); alert("Photo uploaded successfully!"); } catch (err) { alert("Failed to upload photo"); } }} /></label><p className="text-xs text-[#94A3B8] mt-1">JPG or PNG. Max 5MB.</p></div>
                </div>
        ...
        {activeSection === "payout" && (
          <div>
            <div className="mb-8"><h1 className="text-2xl font-bold text-[#1B2A4A] tracking-tight">Payout Settings</h1><p className="text-[#667085] mt-1">Add your bank account details to receive payouts for consultations</p></div>
            <div className="bg-white rounded-xl border border-[#E5EAF0] p-8 max-w-3xl">
              <form onSubmit={handleProfileUpdate} className="space-y-5">
        ...
        {activeSection === "wallet" && (
          <div>
            <div className="mb-8"><h1 className="text-2xl font-bold text-[#1B2A4A] tracking-tight">Wallet</h1><p className="text-[#667085] mt-1">Track your earnings and request payouts</p></div>
            <div className="grid md:grid-cols-3 gap-5 mb-6">
              <div className="bg-white rounded-xl border border-[#E5EAF0] p-6"><p className="text-sm font-medium text-[#667085] uppercase tracking-wide">Balance</p><p className="text-3xl font-bold text-[#1B2A4A] mt-2">₦{wallet?.balance?.toLocaleString() || 0}</p></div>
              <div className="bg-white rounded-xl border border-[#E5EAF0] p-6"><p className="text-sm font-medium text-[#667085] uppercase tracking-wide">Total Earned</p><p className="text-3xl font-bold text-[#5DBB63] mt-2">₦{wallet?.totalEarned?.toLocaleString() || 0}</p></div>
              <div className="bg-white rounded-xl border border-[#E5EAF0] p-6"><p className="text-sm font-medium text-[#667085] uppercase tracking-wide">Pending</p><p className="text-3xl font-bold text-[#F59E0B] mt-2">₦{wallet?.pendingBalance?.toLocaleString() || 0}</p></div>
            </div>
            {/* Request Payout */}
            <div className="bg-white rounded-xl border border-[#E5EAF0] p-8 max-w-lg mb-6">
              <h2 className="text-lg font-bold text-[#1B2A4A] mb-4">Request Payout</h2>
              <p className="text-[#667085] text-sm mb-4">Payouts are processed manually by admin. Ensure your bank details are saved in Payout Settings.</p>
              <div className="flex gap-3">
                <input type="number" value={payoutAmount} onChange={(e) => setPayoutAmount(e.target.value)} placeholder="Amount (₦)" className="flex-1 px-4 py-3 border border-[#E5EAF0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00A6A6]/20 focus:border-[#00A6A6] text-sm" />
                <button onClick={handleRequestPayout} disabled={actionLoading === "payout"} className="bg-[#1B2A4A] text-white px-6 py-3 rounded-lg hover:bg-[#16213A] font-semibold text-sm transition-colors disabled:opacity-50">{actionLoading === "payout" ? "..." : "Request Payout"}</button>
              </div>
            </div>
            {/* Transaction History */}
            <div className="bg-white rounded-xl border border-[#E5EAF0]">
              <div className="p-6 border-b border-[#E5EAF0]"><h2 className="text-lg font-bold text-[#1B2A4A]">Transaction History</h2></div>
              {wallet?.transactions?.length > 0 ? (
                <div className="divide-y divide-[#E5EAF0]">
                  {[...wallet.transactions].reverse().map((tx: any, i: number) => (
                    <div key={i} className="p-4 flex justify-between items-center">
                      <div><p className="text-sm font-medium text-[#1B2A4A]">{tx.description}</p><p className="text-xs text-[#94A3B8]">{new Date(tx.createdAt).toLocaleDateString()} • {tx.type}</p></div>
                      <p className={`text-sm font-semibold ${tx.type === 'credit' ? 'text-[#5DBB63]' : 'text-[#EF4444]'}`}>{tx.type === 'credit' ? '+' : '-'}₦{tx.amount?.toLocaleString()}</p>
                    </div>
                  ))}
                </div>
